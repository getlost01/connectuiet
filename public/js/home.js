...
async function fetchApi(){
    try {
        ...
        var temp = ``;
        for(var i = 0; i<10;++i){temp += `<img src="${(data[i].image)?data[i].image:"./images/user.png"}" alt="${data[i].name.split(" ")[0]} ${data[i].designation}" class="homeImage">`;}
        document.querySelector('#gallery1').innerHTML = temp;
        temp = ``;
        for(var i = 5; i<15;++i){temp += `<img src="${(data[i].image)?data[i].image:"./images/user.png"}" alt="${data[i].name.split(" ")[0]} ${data[i].designation}" class="homeImage">`;}
        document.querySelector('#gallery2').innerHTML = temp;
        temp = ``;
        for(var i = 10; i<20;++i){temp += `<img src="${(data[i].image)?data[i].image:"./images/user.png"}" alt="${data[i].name.split(" ")[0]} ${data[i].designation}" class="homeImage">`;}
